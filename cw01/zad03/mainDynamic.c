...
mainArray_t* (*dlCreateMainArray)();
rowsBlock_t* (*dlCreateRowsBlock)();
FILE* (*dlMergeFiles)();
int (*dlNumberOfRows)();
void (*dlRemoveBlock)();
void (*dlRemoveRow)();
void (*dlPrintMergedBlocks)();
...
void runTest(){
    ...
    for(int i = 0; i < 3; i++){
        for(int j = 0; j < 3; j++){
            ...
            mainArray = dlCreateMainArray(size[i]);
            ...
            for(int k = 0; k < size[i]; k++){                
                mergedFiles[k] = dlMergeFiles("testFileA.txt", "testFileB.txt");
            }
            endReal = clock();
            times(&end);
            ...
            times(&start);
            for(int k = 0; k < size[i]; k++){                
                mainArray->rowsBlocks[k] = dlCreateRowsBlock(mergedFiles[k]);
            }
            endReal = clock();
            times(&end);
...
int main(int argc, char **argv){
    ...
    dlHandler = dlopen("./libmerge.so", RTLD_LAZY);

    dlCreateMainArray = dlsym(dlHandler, "createMainArray");
    dlCreateRowsBlock = dlsym(dlHandler, "createRowsBlock");
    dlMergeFiles = dlsym(dlHandler, "mergeFiles");
    dlNumberOfRows = dlsym(dlHandler, "numberOfRows");
    dlRemoveBlock = dlsym(dlHandler, "removeBlock");
    dlRemoveRow = dlsym(dlHandler, "removeRow");
    dlPrintMergedBlocks = dlsym(dlHandler, "printMergedBlocks");

    if(dlHandler == NULL){
        printf("No library available!\n");
        return -1;
    }
    ...
    if(argc > 2){
        for(int i = 1; i < argc; i++){
            if(strcmp(argv[i], "create_table") == 0){
                ...
                mainArray = dlCreateMainArray(size);
                mainArray->size = size;
                ...
                if(mainArray){
                    ...
                    while(mergedFiles < mainArray->size){
                        ...
                        if(firstFile && secondFile){
                            FILE *merged = dlMergeFiles(firstFile, secondFile);
                            int ptr = 0;
                            while(mainArray->rowsBlocks[ptr] != NULL){
                                ptr = ptr + 1;
                            }
                            mainArray->rowsBlocks[ptr] = dlCreateRowsBlock(merged);
                ...
                if(mainArray){
                    dlRemoveBlock(mainArray, atoi(argv[++i]));
                }
                else{
                    printf("Main array has not been created yet!\n");
                    return -1;
                ...
                if(mainArray){
                    ...
                    dlRemoveRow(mainArray, blockIndex, rowIndex);
                }
                else{
                    printf("Main array has not been created yet!\n");
                    return -1;
